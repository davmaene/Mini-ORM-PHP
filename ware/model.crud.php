...
class CRUD__ implements Metier_
{
    ...
    private function extractWordFrom($chaine)
    {
        $pointPosition = strpos($chaine, ".");

        $sousChaine = substr($chaine, $pointPosition + 1);

        $deuxiemePointPosition = strpos($sousChaine, ".");

        $mot = substr($sousChaine, 0, $deuxiemePointPosition);

        return $mot;
    }
    private function createClassFromString($string)
    {
        $prefixe = "__tbl_";
        $path = "middleware/models/";
        $models = scandir("$path");
        foreach (array_slice($models, 2) as $model) {
            $str = $this->extractWordFrom($model);
            $rpath = ("$path" . "$model");
            if (strtolower($prefixe . $str) === strtolower($string)) {
                $clss = ucfirst($str);
                $clss = new $clss;
                // var_dump($clss);
                return $clss;
    ...
    { // create on instance in to the db
        if (!is_object($this))
    ...
    { // create instance and add record to db
        $conf = new Config();
    ...
    public function delete(array $clauses)
    {
    }
    // -------------------------- CRUD ``` EDIT ``` ---------------
    ...
    public function getOne(array $clauses = null, array $joiture = null, $sort = null, $connection)
    {
        ...
        foreach ($clauses as $key => $value) {
            ++$nblines;
            $value_ = is_numeric($value) && strlen($value) < 3 ? $value : "'" . $value . "'";
            $query .= ((int) $nblines === count($clauses)) ? " `$key` = $value_" : " `$key` = $value_ $connection AND";
        }
        $query .= " LIMIT 1";
        ...
        if ($jointure !== null) {
            if (is_array($jointure)) {
                ...
                foreach ($jointure as $k => $va) {
                    ...
                    $joiTable = $this->createClassFromString($o); // ->__createClass();
                    $joiTableName = $joiTable->__createClass(); // ->__createClass();
                    $handLeft = $va['on'][0];
                    $handRight = $va['on'][1];
                    ...
                    if (isset($o)) { // $va['joinedto']
                        $joinedto = $o; //$va['joinedto'];
                        $query .= " INNER JOIN `$joiTableName` ON $joiTableName.$handRight = $nclassname.$handLeft ";
                    } else $query .= " INNER JOIN `$joiTableName` ON $joiTableName.$handRight = $nclassname.$handLeft ";
                }
                $query .= " $cl ";
            }
        }
                ...
                for ($i = 0; $i < count($rem); $i++) {
                    ...
                    if (is_array($jointure)) {
                        ...
                        for ($o = 0; $o < count($tablesOnJointure); $o++) {
                            ...
                            $toj = $this->createClassFromString($toj);
